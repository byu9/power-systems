...
def _from_earth_tables(symbol, JME):
    coeffs = [
        sum(
            A * cos(B + C * JME)
            for (A, B, C) in table
        )
        for table in _earth_tables[symbol]
    ]
...
def solar(datetime):
    # Julian Day
    JD = pandas.Timestamp(datetime).to_julian_date()
    ...
    L = _from_earth_tables('L', JME) / pi * 180 % 360

    # earth heliocentric latitude (degrees)
    B = _from_earth_tables('B', JME) / pi * 180 % 360

    # earth radius vector (astronomical unit)
    R = _from_earth_tables('R', JME)
    ...
    # longitude of the acending node of the moon's mean orbit on the eliptic,
    ...
    v = v0 + delta_psi * cos(epsilon)

    # sun right ascension (degrees)
    alpha = (
        atan(sin(lambda_) * cos(epsilon) -
             tan(beta)    * sin(epsilon))
    ...
    delta = asin(
        ...
        cos(beta) * sin(epsilon) * sin(lambda_)
    ) / pi * 180
